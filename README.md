...
> Имя `__all__` можно задать в любом файле, не только `__init__.py`. Он позволяет указать конкретный имена для импорта из этого файла. Все остальные имена импортировать не получиться.
...
Надеюсь вам было это полезно! Если нашли ошибки или хотите что-то добавить жду ваших issue!